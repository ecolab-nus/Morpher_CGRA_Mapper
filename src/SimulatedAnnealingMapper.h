...
namespace CGRAXMLCompile
{
	...
	public:
		...
		// by Peng
		void getStringFromDFGAddr(DFG *dfg);
		// generate DFG-node placement using abstract mapping
		void genNodePlacement(CGRA *cgra, DFG *dfg);
		bool getDPFromPE(int t, int x, int y, DataPath **retDP);
		// the final II derived by abstract mapping
		int getAbstractII(CGRA *cgra, DFG *dfg);
		// parse the DFG-node placement generated by abstract mapping
		void parsePlacement();
		// use DFG-node placement from abstract mapping as initial value of SA
		bool iniSARoute(DFGNode *node, DataPath *candidateDP);
		bool initMapWithAbsMap();

		bool SAMap(CGRA *cgra, DFG *dfg, int iniMethod);
		// end by Peng
		...
		bool SARoute(DFGNode *node, DataPath *candidateDP);
		int getCost();
		...
		{
			if(getNumberOfUnmappedNodes()>0) return false;
	...
	private:
		...
		std::map<DFGNode *, DataPath *> absNodePlacement; // node placement from abstract mapping
		string dfgAddress;
	};
