...
{
	DFG tempDFG;
	tempDFG.parseXML(arg.dfg_filename, arg.synthetic_dfg);
	// tempDFG.printDFG();
	this->dfg = &tempDFG;
	...
	{
		DFG dfg;
		dfg.parseXML(arg.dfg_filename, arg.synthetic_dfg);
		...
		if (mapped)
		{
			...
			/* this is to record hoy many steps to achieve the final mapping.
			std::ofstream result_file;
			result_file.open("lisa_log.txt", std::ios_base::app);
			result_file << dfg_id<<" " << total_accepted<<"\n";
			result_file.close(); 
			*/
			break;
		}
	...
	if (!mapped)
	{
		...
		std::cout << "Exceed the maximum II. mapping failed..........." << std::endl;
	}
	start_II = curr_II;
	return mapped;
}
...
void CGRAXMLCompile::LISAMapper::do_training(arguments arg, TimeDistInfo &tdi, int start_II)
{
	...
	for (int traning_iteration = 0; traning_iteration < max_training_iteration; traning_iteration++)
	{
		...
		auto this_iteration_start = std::chrono::steady_clock::now();
		int curr_II = start_II;
		bool mapped = false;
		...
		// mapping
		{
			...
			for (; curr_II < arg.max_II; curr_II++)
			{
				...
				int mapping_time = int(chrono::duration_cast<chrono::seconds>(training_end_time - training_start_time).count() );
				if(mapping_time > training_time_limit){
					break;
				}
				DFG dfg;
				dfg.parseXML(arg.dfg_filename, arg.synthetic_dfg);
				...
				{
					auto end = std::chrono::steady_clock::now();
					std::chrono::duration<double> elapsed_seconds = end - this_iteration_start;
					...
					perf_metric this_iter_perf{curr_II, getCost(), int(elapsed_seconds.count())};
					bool is_best = false;
			...
			{
				auto this_iteration_end = std::chrono::steady_clock::now();
				std::chrono::duration<double> elapsed_seconds = this_iteration_end - this_iteration_start;
				std::cout << "training iteration:" << traning_iteration << "\t not mapped "
						  << "\t running time:" << elapsed_seconds.count() << "\n";
				training_log << "training iteration:" << traning_iteration << "\t not mapped "
							 << "\t running time:" << elapsed_seconds.count() << "\n";
			}
...
bool CGRAXMLCompile::LISAMapper::LISAMapCore(CGRA *cgra, DFG *dfg)
{
	...
	while (curr_temp > minimim_temp)
	{
		...
		if(accept_rate <= 0.02 ) return false;
	}
...
bool CGRAXMLCompile::LISAMapper::initMap()
{
	...
	auto lisa_sortedNodeList = sortedNodeList;
	std::sort(lisa_sortedNodeList.begin(), lisa_sortedNodeList.end(),
	[&](auto a, auto b ){
		int a_idx = a->idx;
		int b_idx = b->idx;
		assert(dfg_label_->find(a_idx) != dfg_label_->end());
		assert(dfg_label_->find(b_idx) != dfg_label_->end());
		int a_asap = dfg_label_->at(a_idx).schedule_order;
		int b_asap = dfg_label_->at(b_idx).schedule_order;
		return a_asap < b_asap;
	});
	...
	while (!unmappedNodes.empty())
	{
		...
		for (int i = 0; i < 20; i++)
		{
			auto dp_candidate = getLISADPCandidate(node);
			if(dp_candidate == nullptr){
	...
	std::cout<<"# of mapped nodes in initial mapping: "<< dfg_node_placement.size()<<" of "<<sortedNodeList.size()<<"\n";
	return true;
}
bool CGRAXMLCompile::LISAMapper::initMap_with_PathFinder()
{
...
float CGRAXMLCompile::LISAMapper::inner_map()
{
	...
	for (int i = 0; i < movement_in_each_temp; i++)
	{
		...
		{
			dp_candidate = getLISADPCandidate(selected_dfg_node, accepted_number, i);
			// dp_candidate = getRandomDPCandidate(selected_dfg_node).front();
		}
		...
		if(route_succ){
			LOG(SA)<<"route success \t"<<"temp:"<<curr_temp<<" cost:"<<(attempted_cost-curr_cost)<<" "<< "accept " << (accept ? "ture" : "false")<<"\n"; 
		}else{
...
CGRAXMLCompile::DataPath *CGRAXMLCompile::LISAMapper::getLISADPCandidate(DFGNode *dfg_node, int accepted, int total_tried)
{
	...
	for (auto node : candidateDests)
	{
		...
		for (auto node_ass : ass)
		{
			...
			if (dumped_mapping.find(node_id) == dumped_mapping.end())
				continue;
	...
	if (total_tried > 0)
	{
		deviation = 0.5 * (total_tried)-accepted;
		deviation = std::max(deviation, 1.0);
	}

	std::normal_distribution<> d{0, deviation};
	int selected_cost = std::abs(std::round(d(gen)));
	LOG(LISADETAIL) << "initial cost:" << selected_cost;
	if (selected_cost > max_diff)
		selected_cost = max_diff;
	...
	LOG(LISADETAIL) << output.str();
	LOG(LISADETAIL) << "label of " << dfg_node->idx << "  " << dfg_node_label.toStr();
	LOG(LISADETAIL) << "derivation:" << deviation << "\t total_tried:" << total_tried << "\t accepted:" << accepted;
	LOG(LISADETAIL) << "max_diff: " << max_diff << "\t max cost:" << max_cost << "\t min_cost:" << min_cost;
	LOG(LISADETAIL) << " min cost:" << min_cost << "   selected cost: " << selected_cost + 1 << "\n";
	LOG(LISADETAIL) << "lisa op:" << dfg_node->idx << " selectFU: " << selected_fu->getFullName() << "\n";
	LOG(LISADETAIL) << "curr mapping: " << dumpMappingToStr();
	LOG(LISADETAIL) << "best mapping: " << lisa_ctrl->bestMappingToStr();
	assert(selected_fu != nullptr);
	return selected_fu;
}
...
std::vector<CGRAXMLCompile::DataPath *> CGRAXMLCompile::LISAMapper::getCandidateByIIConstraint(int start_II, int end_II, DFGNode *node)
{
	...
	std::vector<DataPath*> selected_candidates;
	for(auto node: candidates){
		auto node_t = node->get_t();
		if(node_t >= start_II && node_t <= end_II){
...
CGRAXMLCompile::DataPath *CGRAXMLCompile::LISAMapper::getCloseRandomDP(DFGNode *dfg_node, DataPath *old_dp, int max_physical_dis, int max_temp_dis)
{
	...
	{
		if (std::abs(old_dp->getPE()->getPosition_X() - dp->getPE()->getPosition_X()) + std::abs(old_dp->getPE()->getPosition_Y() - dp->getPE()->getPosition_Y()) <= max_physical_dis && allowed_cycle.find(dp->get_t()) != allowed_cycle.end())
		{
	...
	if (opt_candidates.size() == 0)
	{
		...
		LOG(LISADETAIL) << "  select fu:" << sel_dp->getFullName();
		return sel_dp;
	}
...
std::pair<int, int> CGRAXMLCompile::LISAMapper::getIntervalByScheduleOrder(std::map<int, pos3d> &dumped_mapping, DFGNode *dfg_node, int scheduler_order)
{
	...
	for (auto node : sortedNodeList)
	{
		...
		if (dfg_label_->at(node->idx).schedule_order < scheduler_order && dfg_node_placement.find(node) != dfg_node_placement.end())
			early_ops.insert(node);
		if (dfg_label_->at(node->idx).schedule_order > scheduler_order && dfg_node_placement.find(node) != dfg_node_placement.end())
			late_ops.insert(node);
	}
...
std::map<CGRAXMLCompile::DataPath *, int> CGRAXMLCompile::LISAMapper::getCostByComm(std::map<int, pos3d> &dumped_mapping, std::vector<DataPath *> candidates, DFGNode *node)
{
	...
	auto find_routing_resource_ = [&](DataPath *a, bool up_directoin)
	{
		...
		while (true)
		{
			...
			for (auto node : curr_routing_resource)
			{
				...
				if (x - 1 < 0)
					blocked_side++;
				if (x + 1 >= this->cgra->get_x_max())
					blocked_side++;
				if (y - 1 < 0)
					blocked_side++;
				if (y + 1 >= this->cgra->get_y_max())
					blocked_side++;
				if (blocked_side >= 2)
				{
					break;
...
std::map<CGRAXMLCompile::DataPath *, int> CGRAXMLCompile::LISAMapper::getCostByAssociation(std::map<int, pos3d> &dumped_mapping, std::vector<DataPath *> candidates, DFGNode *dfg_node, int eraly_lat)
{
	auto &ass = dfg_label_->at(dfg_node->idx).association;
	...
	auto get_cost = [&, this](DataPath *a)
	{
		...
		for (auto node_ass : ass)
		{
			...
			mapped_ass_node++;
			auto &m = dumped_mapping[node_id];
			total_spatial_cost += std::abs(std::abs(node_ass.second.first) - (std::abs(a->getPE()->getPosition_X() - m.x) + std::abs(a->getPE()->getPosition_Y() - m.y)));
			total_temp_cost += std::abs(m.t - guess_time);
		}
		if (mapped_ass_node == 0)
...
std::map<CGRAXMLCompile::DataPath *, int> CGRAXMLCompile::LISAMapper::getCostForSameLevelNode(std::map<int, pos3d> &dumped_mapping, std::vector<DataPath *> candidates, DFGNode *dfg_node)
{
	...
	{
		if (mapped_sameLevel_nodes.size() == 0)
			return 0;
		int total_cost = 0;
